...
{
    protected $table = 'xjc_shop_goods_category';
    protected $type = [
        'more' => 'array',
    ];
     ...
     * @return string
     */
    public function adminGoodsCategoryTree($selectId = 0, $currentCid = 0)
    {
        $where = ['delete_time' => 0];
        if (!empty($currentCid)) {
     ...
     * @return string
     */
    public function adminGoodsCategoryTableTree($currentIds = 0, $tpl = '')
    {
        $where = ['delete_time' => 0];
//        if (!empty($currentCid)) {
//            $where['id'] = ['neq', $currentCid];
//        }
        ...
        foreach ($categories as $item) {
            ...
            $item['url']     = cmf_url('shop/List/index', ['id' => $item['id']]);;
            $item['str_action'] = '<a href="' . url("AdminGoodsCategory/add", ["parent" => $item['id']]) . '">添加子分类</a>  <a href="' . url("AdminGoodsCategory/edit", ["id" => $item['id']]) . '">' . lang('EDIT') . '</a>  <a class="js-ajax-delete" href="' . url("AdminGoodsCategory/delete", ["id" => $item['id']]) . '">' . lang('DELETE') . '</a> ';
            array_push($newCategories, $item);
        }
